(() => {
  ...
  function showNotification(message, type = 'success') {
    ...
    el.style.cssText = `
      ...
      background:${colors[type] || colors.success};color:white;max-width:320px;
    `;
  ...
  async function triggerAutofill() {
    ...
    try {
      ...
      if (response.error) {
        const msgs = {
          NO_PROFILE: 'Please set up a profile first.',
  ...
  chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    if (message.type === 'TRIGGER_AUTOFILL') { triggerAutofill(); sendResponse({}); }
